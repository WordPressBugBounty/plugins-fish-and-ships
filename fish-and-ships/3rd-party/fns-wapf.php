		...
		function wc_fns_get_selection_methods_fn($methods = array()) {
			...
			else
			{
				foreach ( $fields as $id => $field_data ) {
					...
					$methods[ 'wapf-' . $id ] = array( 'onlypro' => false, 'group' => 'Adv. Product Fields', 'scope' => $scope_all,  'label' => $field_data[ 'group_name' ] . ' &gt; ' . $field_data[ 'label' ] );					
				}
			}
			return $methods;
		}
					...
					case 'calc':
						...
						{	
							...
							if ( $product_field[ 'id' ] == $field_data['id'] )
							{
								/*
								error_log('====WAPF===');
								error_log('field_data: ');
								error_log(print_r($field_data, true));
								error_log('product_field: ');
								error_log(print_r($product_field, true));
								error_log('====/WAPF===');
								*/
